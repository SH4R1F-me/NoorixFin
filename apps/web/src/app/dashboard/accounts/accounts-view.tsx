...
export default function AccountsView({
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    cardInfo: { flex: 1 },
    cardName: { fontSize: '0.9375rem', fontWeight: 600, color: 'var(--text-primary)' },
    cardSubtype: { fontSize: '0.75rem', color: 'var(--text-tertiary)', marginTop: 1 },
    cardBalance: {
      fontSize: '1.25rem',
      fontWeight: 800,
      fontVariantNumeric: 'tabular-nums' as const,
  ...
  return (
    <div>
      <div style={s.hdr}>
        <div>
          <h1 style={s.title}>{t('accounts.title')}</h1>
        </div>
      ...
      {showCreate && (
        <div style={s.createBox}>
          <div style={s.grid}>
            <div style={s.fg}>
              <label style={s.lbl} htmlFor="acc-name">
            ...
            <div style={s.fg}>
              <label style={s.lbl} htmlFor="acc-subtype">
      ...
      {accounts.length === 0 && !showCreate && (
        <p style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem', margin: '2rem 0' }}>
      ...
      <div style={s.summaryGrid}>
        <div style={s.summaryCard}>
          ...
          <span style={{ ...s.summaryValue, color: 'var(--color-primary-500)' }}>
            {mask(fmt(totalAssets))}
          </span>
        </div>
        <div style={s.summaryCard}>
          <span style={s.summaryLabel}>{t('accounts.liability')}</span>
          <span style={{ ...s.summaryValue, color: 'var(--color-error)' }}>
            {mask(fmt(totalLiabilities))}
          </span>
        </div>
        <div style={s.summaryCard}>
          ...
          <span
            style={{
              ...s.summaryValue,
              color: netWorth >= 0 ? 'var(--color-primary-500)' : 'var(--color-error)',
            }}
          >
            {mask(fmt(netWorth))}
          </span>
      ...
      <div style={s.section}>
        <div style={s.sectionTitle}>
          <TrendingUp size={16} style={{ color: 'var(--color-primary-500)' }} /> Assets (
          {assets.length})
        </div>
        <div style={s.cardGrid}>
          {assets.map((acc) => (
            <div key={acc.id} style={s.card}>
              ...
              <div style={{ ...s.cardBalance, color: 'var(--color-primary-500)' }}>
      ...
      {liabilities.length > 0 && (
        <div style={s.section}>
          <div style={s.sectionTitle}>
            <CreditCard size={16} style={{ color: 'var(--color-error)' }} /> Liabilities (
            {liabilities.length})
          </div>
          <div style={s.cardGrid}>
            {liabilities.map((acc) => (
              <div key={acc.id} style={s.card}>
                ...
                <div style={{ ...s.cardBalance, color: 'var(--color-error)' }}>
                  {mask(fmt(acc.balance, acc.currency))}
                </div>
              </div>
            ))}
